...
for ref = 1 : ref_cnt
    ...
    if isempty(found)
        ...
        %
        % referred page in memory > no page fault  
        % - re-sort list as per LRU order 
        % - take referred page to index nphysical (head of list)
        inmem = [ inmem(1:found-1) inmem(found+1:nphysical) inmem(found) ];
    end;
end;
end
% ------------------------------------------------------------------%
